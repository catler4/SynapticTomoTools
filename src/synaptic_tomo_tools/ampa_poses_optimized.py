...
def find_optimal_ampa_pairs_networkx(aunp_coordinates, postsynaptic_data,
                                    inter_aunp_distance=(6, 12),
                                    aunp_membrane_distance=(17, 23),
                                    ampa_steric_radius=5.0):
    """
    Find the absolute optimal AMPA pairs using NetworkX maximum independent set.
    
    This method constructs a conflict graph where nodes represent valid AuNP pairs
    and edges represent conflicts (shared AuNPs or steric clashes). The maximum
    independent set gives the optimal solution.
    ...
    """
    print("🔬 Using NetworkX exact optimization method...")
    ...
    if len(valid_pairs) > 500:
        print(f"⚠️  Warning: {len(valid_pairs)} pairs is large for exact solution.")
        print("   Consider using greedy method for better performance.")
    
    print("🕸️  Building conflict graph...")
    
    # Create conflict graph
    G = nx.Graph()
    
    # Add nodes for each valid pair
    for i, pair in enumerate(valid_pairs):
        G.add_node(i, pair=pair)
    
    # Add edges for conflicts
    print("🔗 Identifying conflicts between pairs...")
    conflicts_found = 0
    ...
    print(f"   Found {conflicts_found} conflicts between pairs")
    
    # Find maximum independent set
    print("🎯 Finding maximum independent set (optimal solution)...")
    print(f"   Graph has {len(valid_pairs)} nodes and {conflicts_found} edges")
    print("   This may take several minutes for large graphs...")
    
    try:
        # Use maximum clique on complement graph (equivalent to maximum independent set)
        ...
        def timeout_handler(signum, frame):
            raise TimeoutError("NetworkX optimization timed out")
        
        # Set timeout based on problem size
        if len(valid_pairs) <= 500:
            timeout_seconds = 1800  # 30 minutes
        elif len(valid_pairs) <= 1000:
            timeout_seconds = 1800  # 30 minutes
        else:
            timeout_seconds = 1800  # 30 minutes
        
        print(f"   Setting timeout to {timeout_seconds//60} minutes...")
        ...
        print("   Starting NetworkX optimization...")
        ...
        def progress_monitor():
            ...
            while elapsed < timeout_seconds:
                ...
                if elapsed < timeout_seconds:
                    print(f"   ⏳ Still optimizing... ({elapsed:.0f}s elapsed)")
        
        progress_thread = threading.Thread(target=progress_monitor, daemon=True)
        progress_thread.start()
        
        try:
            max_clique_nodes, max_clique_weight = nx.max_weight_clique(complement_G, weight=None)
            signal.alarm(0)  # Cancel timeout
            
            elapsed_time = time.time() - start_time
            print(f"   ✅ NetworkX optimization completed in {elapsed_time:.1f} seconds")
            
        except TimeoutError:
            signal.alarm(0)  # Cancel timeout
            elapsed_time = time.time() - start_time
            print(f"   ⏰ NetworkX optimization timed out after {elapsed_time:.1f} seconds")
            raise TimeoutError("NetworkX optimization timed out")
        
        # Extract optimal pairs
        optimal_pairs = [valid_pairs[node] for node in max_clique_nodes]
        ...
        if isinstance(e, TimeoutError):
            print(f"❌ NetworkX optimization timed out: {e}")
        else:
            print(f"❌ Error in NetworkX optimization: {e}")
        print("   NetworkX optimization failed. Please try using the greedy method instead.")
        return {
            'pairs': [],
            'ampa_positions': [],
            'ampa_orientations': [],
...
def estimate_ampa_poses_optimized(
    ...
    Args:
        ...
        method: Optimization method ("greedy", "networkx", or "ilp")
    ...
    if method == "networkx":
        optimal_solution = find_optimal_ampa_pairs_networkx(
    ...
    if method == "greedy":
        ...
        # For ILP and NetworkX, set default values
        top_3_solutions = []
...
from .ampa_poses import load_postsynaptic_coordinates, run_ampa_poses_analysis
...
def run_ampa_poses_analysis_optimized(tomo_path, output_dir, aunp_active_zones=None,
    ...
    Args:
        ...
        aunp_membrane_distance: Tuple of (min, max) distance from AuNP to membrane in nm
        ampa_steric_radius: Minimum distance between AMPA positions in nm
        method: Optimization method ("greedy" or "networkx")
    ...
    if method == "networkx":
        if "networkx" in output_dir_str:
            method_output_dir = Path(output_dir)
            print(f"📁 Using NetworkX method - output directory: {method_output_dir}")
        else:
            method_output_dir = Path(output_dir) / "networkx"
            print(f"📁 Using NetworkX method - output directory: {method_output_dir}")
    elif method == "ilp":
