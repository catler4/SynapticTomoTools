...
def analyze_aunps(tomogram_path, active_zone_indices=None, set_name=None):
    ...
    try:
        ...
        try:
            # Use DBSCAN with min_samples=2, then filter out clusters with < 4 points
            ...
            for label, count in zip(unique_labels, counts):
                if label == -1:  # Keep noise as noise
                    ...
                    label_mapping[label] = -1
                else:  # Keep larger clusters as they are
            ...
            print(f"DBSCAN found {n_clusters} AuNP clusters (eps=20 nm, min_samples=2)")
