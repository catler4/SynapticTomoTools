...
def write_aunp_spheres_pdb(aunp_positions: List[np.ndarray], 
    ...
    try:
        with open(output_path, 'w') as f:
            ...
            # Write each AuNP as a sphere (represented by a single atom)
            for i, position in enumerate(aunp_positions):
                # Use HETATM for non-standard atoms
                f.write(f"HETATM{i+1:5d}  AU   AUN {i+1:4d}    {position[0]:8.3f}{position[1]:8.3f}{position[2]:8.3f}  1.00 20.00          AU\n")
