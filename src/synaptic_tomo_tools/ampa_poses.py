...
def download_pdb_structure(pdb_id: str, output_path: Path) -> bool:
    ...
    try:
        ...
        print(f"Downloaded PDB structure {pdb_id} to {output_path}")
        return True
    except Exception as e:
        print(f"Error downloading PDB structure {pdb_id}: {e}")
        return False


def parse_pdb_coordinates(pdb_path: Path) -> np.ndarray:
    """
    ...
    Returns:
        Array of atomic coordinates (N, 3)
    """
    coordinates = []
...
def transform_pdb_structure(coordinates: np.ndarray, position: np.ndarray, 
                          orientation: np.ndarray) -> np.ndarray:
    """
    Transform PDB coordinates to new position and orientation.
    
    Args:
        coordinates: Original PDB coordinates (N, 3) - should be pre-aligned to reference vectors
        position: Target position (3,)
        orientation: Euler angles in degrees (3,) - ZYZ convention from STAR file
        
    Returns:
        Transformed coordinates (N, 3)
    """
    # Apply the INVERSE rotation (confirmed to be correct)
    # The STAR file angles represent the rotation FROM reference TO observed
    # But we need to rotate the PDB TO match the observed orientation
    
    center_of_mass = np.mean(coordinates, axis=0)
    centered_coords = coordinates - center_of_mass
    
    # Apply the inverse of the calculated rotation
    rotation = R.from_euler('ZYZ', orientation, degrees=True)
    inverse_rotation = rotation.inv()
    rotated_coords = inverse_rotation.apply(centered_coords)
    
    # Apply translation to final position
    transformed_coords = rotated_coords + position
    
    return transformed_coords


def write_aunp_spheres_pdb(aunp_positions: List[np.ndarray], 
                          output_path: Path,
                          tomo_name: str,
                          sphere_diameter: float = 2.0) -> bool:
    """
    Write PDB file with AuNP positions represented as spheres.
    
    Args:
        aunp_positions: List of AuNP positions (N, 3) in Angstroms
        output_path: Path to save output PDB file
        tomo_name: Name of tomogram
        sphere_diameter: Diameter of spheres in nm (default: 2.0)
        
    Returns:
        True if successful, False otherwise
    """
    try:
        # Convert sphere diameter from nm to Angstroms
        sphere_radius_angstroms = (sphere_diameter / 2.0) * 10  # nm to Å
        
        with open(output_path, 'w') as f:
            # Write header
            f.write(f"HEADER    AUNP POSITIONS FROM TOMOGRAM {tomo_name}\n")
            f.write(f"REMARK    GENERATED BY SYNAPTIC TOMO TOOLS\n")
            f.write(f"REMARK    {len(aunp_positions)} AUNP SPHERES (DIAMETER {sphere_diameter} NM)\n")
            f.write(f"REMARK    EACH SPHERE REPRESENTED BY 8 ATOMS IN OCTAHEDRAL PATTERN\n")
            f.write("\n")
            
            atom_index = 1
            # Write each AuNP as a sphere (represented by 8 atoms in octahedral pattern)
            for i, position in enumerate(aunp_positions):
                # Create 8 atoms in octahedral pattern around the center
                offsets = [
                    [sphere_radius_angstroms, 0, 0],
                    [-sphere_radius_angstroms, 0, 0],
                    [0, sphere_radius_angstroms, 0],
                    [0, -sphere_radius_angstroms, 0],
                    [0, 0, sphere_radius_angstroms],
                    [0, 0, -sphere_radius_angstroms],
                    [sphere_radius_angstroms/1.4, sphere_radius_angstroms/1.4, 0],
                    [-sphere_radius_angstroms/1.4, -sphere_radius_angstroms/1.4, 0]
                ]
                
                for j, offset in enumerate(offsets):
                    atom_pos = position + np.array(offset)
                    # Use HETATM for non-standard atoms
                    f.write(f"HETATM{atom_index:5d}  AU   AUN {i+1:4d}    {atom_pos[0]:8.3f}{atom_pos[1]:8.3f}{atom_pos[2]:8.3f}  1.00 20.00          AU\n")
                    atom_index += 1
            
            f.write("END\n")
        
        print(f"  ✅ Saved AuNP spheres PDB file to {output_path}")
        return True
        
    except Exception as e:
        print(f"  ❌ Error writing AuNP spheres PDB: {e}")
        return False


def write_pdb_with_poses(ampa_positions: List[np.ndarray], 
                        ampa_orientations: List[np.ndarray],
                        pdb_template_path: Path,
                        output_path: Path,
                        tomo_name: str) -> bool:
    """
    Write PDB file with AMPA structures at calculated poses.
    
    Args:
        ampa_positions: List of AMPA positions (N, 3) in Angstroms
        ampa_orientations: List of AMPA orientations (N, 3) - Euler angles
        pdb_template_path: Path to template PDB file (7LDD)
        output_path: Path to save output PDB file
        tomo_name: Name of tomogram for chain IDs
        
    Returns:
        True if successful, False otherwise
        
    Note:
        AMPA positions should be in Angstroms (Å) to match PDB format.
        Tomogram coordinates are in nm, so convert with: pos_angstroms = pos_nm * 10
    """
    try:
        # Parse template PDB coordinates and atom lines
        template_coords = []
        template_lines = []
        
        with open(pdb_template_path, 'r') as template_file:
        ...
        with open(output_path, 'w') as f:
            # Write header
            f.write(f"HEADER    AMPA RECEPTOR POSES FROM TOMOGRAM {tomo_name}\n")
            f.write(f"REMARK    GENERATED BY SYNAPTIC TOMO TOOLS\n")
            f.write(f"REMARK    BASED ON PDB STRUCTURE 7LDD\n")
            f.write(f"REMARK    {len(ampa_positions)} AMPA RECEPTORS PLACED\n")
            f.write("\n")
            
            # Write each AMPA structure
            for i, (position, orientation) in enumerate(zip(ampa_positions, ampa_orientations)):
                # Transform coordinates
                transformed_coords = transform_pdb_structure(template_coords, position, orientation)
                
                atom_index = 1
                for j, (line, transformed_coord) in enumerate(zip(template_lines, transformed_coords)):
                    try:
                        # Write transformed atom line, preserving the original chain ID from the template
                        new_line = line[:30] + f"{transformed_coord[0]:8.3f}{transformed_coord[1]:8.3f}{transformed_coord[2]:8.3f}" + line[54:]
                        # Keep the original chain ID from the template line (don't overwrite it)
                        new_line = new_line[:6] + f"{atom_index:5d}" + new_line[11:]
        ...
        print(f"  ✅ Saved AMPA poses PDB file to {output_path}")
        return True
...
def find_optimal_ampa_pairs_ilp(aunp_coordinates, postsynaptic_data,
    ...
    Args:
        ...
        ampa_steric_radius: Minimum distance between particle positions
    ...
    try:
        ...
        if pulp.LpStatus[prob.status] == 'Optimal':
            ...
            for pair in optimal_pairs:
                ...
                # Debug: Print the vectors being used for alignment
                print(f"  Reference vectors: [[-0.5, 0, 1], [0.5, 0, 1]] (nm)")
                print(f"  AuNP vectors: [{vector_aunp1}, {vector_aunp2}] (nm)")
                print(f"  Vector magnitudes: {np.linalg.norm(vector_aunp1):.2f}, {np.linalg.norm(vector_aunp2):.2f} nm")
                
                rot, _ = R.align_vectors([[-0.5, 0, 1], [0.5, 0, 1]], [vector_aunp1, vector_aunp2])
                euler_angles = rot.as_euler("ZYZ", degrees=True)
                print(f"  Calculated Euler angles: Rot={euler_angles[0]:.1f}°, Tilt={euler_angles[1]:.1f}°, Psi={euler_angles[2]:.1f}°")
...
def estimate_ampa_poses_optimized(
    ...
    # Save PDB file with AMPA structures at calculated poses (at the very end)
    if len(optimal_solution['ampa_positions']) > 0 and pdb_file:
        print(f"🧬 Generating PDB file with AMPA structures...")
        
        # Use the provided PDB file
        pdb_template_path = Path(pdb_file)
        if pdb_template_path.exists():
            
            # Generate PDB file with AMPA poses
            pdb_output_path = output_path / f"{output_filename}_ampa_poses.pdb"
            # Convert AMPA positions from nm to Angstroms for PDB format
            ampa_positions_angstroms = [pos * 10 for pos in optimal_solution['ampa_positions']]
            
            success = write_pdb_with_poses(
                ampa_positions_angstroms,
                optimal_solution['ampa_orientations'],
                pdb_template_path,
                pdb_output_path,
                tomo_name
            ...
            if success:
                # Also generate AuNP spheres PDB
                aunp_spheres_path = output_path / f"{output_filename}_aunp_spheres.pdb"
                # Convert AuNP positions from nm to Angstroms
                aunp_positions_angstroms = [pos * 10 for pos in aunp_coordinates]
                
                write_aunp_spheres_pdb(
                    aunp_positions_angstroms,
                    aunp_spheres_path,
                    tomo_name,
                    sphere_diameter=2.0
                )
            ...
            if success:
                print(f"  ✅ Saved AMPA poses PDB file to {pdb_output_path}")
            else:
                print("  ❌ Failed to generate PDB file")
        else:
            print(f"  ❌ PDB file not found: {pdb_file}")
    elif len(optimal_solution['ampa_positions']) > 0 and not pdb_file:
        print("  ⚠️  Skipping PDB generation - no PDB file provided")
    ...
    return {
        ...
        "aunps_file": str(output_path / f"{output_filename}_paired_aunps.star"),  # Alias for compatibility
        ...
        "all_aunps_file": str(output_path / f"{output_filename}_paired_aunps.star"),  # Alias for compatibility
...
def run_ampa_poses_analysis_original(tomo_path, output_dir, aunp_active_zones=None,
    ...
    This method generates AMPA poses for ALL valid AuNP pairs without optimization.
    No steric clash avoidance or maximum matching is performed.
    ...
    print(f"🧬 Running original AMPA poses analysis (all_poses) for {tomo_name}")
    ...
    for star_file in aunp_files:
        try:
            ...
            print(f"⚠️  Warning: Could not read {star_file}: {e}")
            continue
    
    if not aunp_data_list:
        print("❌ No valid AuNP data found")
        return {"status": "error", "message": "No valid AuNP data found"}
    ...
    print(f"  📊 Loaded {len(postsynaptic_data)} postsynaptic membrane vertices")
    ...
    if aunp_membrane_distance is None:
        membrane_str = "mem_all"
    else:
        membrane_min, membrane_max = aunp_membrane_distance
        membrane_str = f"mem{membrane_min}-{membrane_max}nm"
    
    output_filename = f"{tomo_name}_ampa_poses_original_{aunp_str}_{membrane_str}"
    
    print(f"🚀 Starting original analysis with {len(aunp_coordinates)} AuNPs and {len(postsynaptic_data)} membrane vertices")
    print(f"📋 Parameters: AuNP distance {inter_aunp_distance}, Membrane distance {aunp_membrane_distance}")
    print()
    
    # Find all valid AuNP pairs
    print("🔍 Finding all valid AuNP pairs...")
    postsynaptic_tree = KDTree(postsynaptic_data)
    tree = KDTree(aunp_coordinates)
    ...
    print(f"✅ Found {len(valid_pairs)} valid AuNP pairs")
    
    if len(valid_pairs) == 0:
        print("❌ No valid AuNP pairs found")
        return {"status": "no_pairs", "message": "No valid AuNP pairs found"}
    
    # Generate AMPA poses for ALL valid pairs (no optimization)
    print("🧮 Calculating AMPA positions and orientations for all valid pairs...")
    ampa_positions = []
    ampa_orientations = []
    results_relion = []
    aunps_used_data = []
    ...
    for pair in valid_pairs:
        ...
        # Calculate distances for RELION format
        aunp_separation = distance.euclidean(aunp1_coord, aunp2_coord)
        aunp1_membrane_distance = postsynaptic_tree.query(aunp1_coord)[0]
        aunp2_membrane_distance = postsynaptic_tree.query(aunp2_coord)[0]
        ampa_membrane_distance = postsynaptic_tree.query(ampa_pos)[0]
    ...
    print(f"✅ Generated {len(ampa_positions)} AMPA poses")
    ...
    unpaired_aunps = [i for i in range(len(aunp_coordinates)) if i not in used_aunps]
    print(f"📊 {len(unpaired_aunps)} AuNPs remain unpaired")
    
    # Save RELION star file
    starfile.write({
        'particles': pd.DataFrame(results_relion),
        'optics': pd.DataFrame([{'rlnOpticsGroup': 1}])
    }, output_dir / f"{output_filename}.star")
    print(f"Saved original RELION star file to {output_dir / f'{output_filename}.star'}")
    ...
    if aunps_used_data:
        starfile.write({
            'particles': pd.DataFrame(aunps_used_data),
            'optics': pd.DataFrame([{'rlnOpticsGroup': 1}])
        }, output_dir / f"{output_filename}_paired_aunps.star")
        print(f"Saved paired AuNPs from original analysis to {output_dir / f'{output_filename}_paired_aunps.star'}")
    
    # Save unpaired AuNPs
    unpaired_aunps_data = []
    for aunp_idx in unpaired_aunps:
        coord = aunp_coordinates[aunp_idx]
        membrane_distance = postsynaptic_tree.query(coord)[0]
        unpaired_aunps_data.append({
            'rlnTomoName': tomo_name,
    ...
    if unpaired_aunps_data:
        starfile.write({
            'particles': pd.DataFrame(unpaired_aunps_data),
            'optics': pd.DataFrame([{'rlnOpticsGroup': 1}])
        }, output_dir / f"{output_filename}_unpaired_aunps.star")
        print(f"Saved {len(unpaired_aunps_data)} unpaired AuNPs to {output_dir / f'{output_filename}_unpaired_aunps.star'}")
    
    # Calculate steric clashes (for comparison with optimized method)
    steric_clashes = check_ampa_steric_clash(ampa_positions, min_distance=5.0)
    print(f"⚠️  Found {len(steric_clashes)} steric clashes (poses too close together)")
    
    # Save summary
    summary_data = [{
        'Method': 'Original (All Poses)',
        'Total_Pairs': len(valid_pairs),
        'Total_AuNPs': len(aunp_coordinates),
        'Paired_AuNPs': len(used_aunps),
        'Unpaired_AuNPs': len(unpaired_aunps),
        'Steric_Clashes': len(steric_clashes),
        'Pairing_Efficiency': len(used_aunps) / len(aunp_coordinates) * 100 if len(aunp_coordinates) > 0 else 0,
        'Inter_AuNP_Distance': str(inter_aunp_distance),
        'Membrane_Distance': str(aunp_membrane_distance)
    }]
    
    summary_df = pd.DataFrame(summary_data)
    summary_df.to_csv(output_dir / f"{output_filename}_summary.csv", index=False)
    print(f"Saved original AMPA poses summary to {output_dir / f'{output_filename}_summary.csv'}")
    
    # Save PDB file with AMPA structures at calculated poses (at the very end)
    if len(ampa_positions) > 0 and pdb_file:
        print(f"🧬 Generating PDB file with AMPA structures...")
        ...
        if pdb_template_path.exists():
            ...
            # Generate PDB file with AMPA poses
            pdb_output_path = output_dir / f"{output_filename}_ampa_poses.pdb"
            # Convert AMPA positions from nm to Angstroms for PDB format
            ampa_positions_angstroms = [pos * 10 for pos in ampa_positions]
            ...
            if success:
                # Also generate AuNP spheres PDB
                aunp_spheres_path = output_dir / f"{output_filename}_aunp_spheres.pdb"
                # Convert AuNP positions from nm to Angstroms
                aunp_positions_angstroms = [pos * 10 for pos in aunp_coordinates]
            ...
            print(f"  ❌ PDB file not found: {pdb_file}")
    elif len(ampa_positions) > 0 and not pdb_file:
        print("  ⚠️  Skipping PDB generation - no PDB file provided")
    
    print(f"✅ Original AMPA poses analysis completed successfully!")
    print(f"Found {len(valid_pairs)} AMPA receptor poses")
    print(f"RELION star file: {output_dir / f'{output_filename}.star'}")
    print(f"Paired AuNPs star file: {output_dir / f'{output_filename}_paired_aunps.star'}")
    print(f"Unpaired AuNPs star file: {output_dir / f'{output_filename}_unpaired_aunps.star'}")
    print(f"Summary CSV: {output_dir / f'{output_filename}_summary.csv'}")
    ...
    return {
        ...
        "unpaired_aunps": len(unpaired_aunps),
        "steric_clashes": len(steric_clashes),
        "star_file": str(output_dir / f"{output_filename}.star"),
        "paired_aunps_file": str(output_dir / f"{output_filename}_paired_aunps.star"),
        "aunps_file": str(output_dir / f"{output_filename}_paired_aunps.star"),  # Alias for compatibility
        "unpaired_file": str(output_dir / f"{output_filename}_unpaired_aunps.star"),
        "all_aunps_file": str(output_dir / f"{output_filename}_paired_aunps.star"),  # Alias for compatibility
        "summary_file": str(output_dir / f"{output_filename}_summary.csv"),
        "particles_data": results_relion,
        "aunps_data": aunps_used_data
    }
