...
Before running analysis modules, tilt series should first be motion corrected and aligned (we typically use MotionCor and Etomo) and then reconstructed and further processed using the [FindingAMPA](https://github.com/jojoelfe/findingampa) preprocessing pipeline (which is a standalone repo, or is installed as a python package within the SynapticTomoTools environment - still beta). Within the findingampa pipeline, tomogram reconstruction (with Etomo), denoising (DeepDeWedge), membrane segmentation (Membrain), membrane and vesicle annotation (wrapper to Blender-based manual annotation setup), and AuNP picking (template-matching approach constrained to cleft regions) need to be completed. These are currently set up to run on the Gouaux lab ophelia3 workstation.
